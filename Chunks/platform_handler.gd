...
func create(position):
	...
	var random_x = rng.randf_range(-20.0, 20.0)
	var random_y = rng.randf_range(-20.0, 20.0)
	platform_instance.position.y = position.y - random_y + 10
	platform_instance.position.x = position.x - random_x
	platform_instance.timer = timer
